...
(async()=>{
  ...
  const prR=await sb('profiles?select=id,abonnement,fin_periode');
  const now=new Date(),premium=new Set();
  (prR.ok?await prR.json():[]).forEach(p=>{if(p.abonnement==='actif'&&(!p.fin_periode||new Date(p.fin_periode)>now))premium.add(p.id);});
