...
async function build(){
  ...
  for(const game of games){
    ...
    matchDataMap[game.gamePk]={away:slim(away.team),home:slim(home.team),heure:time,dateCourt,state:game.status.abstractGameState,probaAway:pA,probaHome:pH,scoreA:rA.score,scoreH:rH.score,confScore:cScore,marge:margeCI,aData,hData,aStand,hStand,aPitStat,hPitStat,aForme,hForme,aSplit,hSplit,meteo,meteoIdx:wind,vegas,sim,parkPct,nomPartantA:away.probablePitcher?.fullName,nomPartantH:home.probablePitcher?.fullName};
    const base={gamePk:game.gamePk,heure:time,dateCourt,matchLabel:matchLbl,awayTeam:slim(away.team),homeTeam:slim(home.team),probaAway:pA,probaHome:pH,state:game.status.abstractGameState,meteoHTML,compoHTML};
    // 1. VICTOIRE
