...
async function getSplits(teamId){
  const out={hitH:null,hitA:null,pitH:null,pitA:null,opsVsL:null,opsVsR:null,opsRecent:null};
  ...
  // PLATOON : OPS de l'attaque vs lanceurs gauchers (vl) / droitiers (vr)
  try{const dpl=await api(`/teams/${teamId}/stats?stats=statSplits&sitCodes=vl,vr&group=hitting&season=${SEASON}`);(dpl.stats?.[0]?.splits??[]).forEach(s=>{if(s.split?.code==='vl')out.opsVsL=parseFloat(s.stat?.ops)||null;if(s.split?.code==='vr')out.opsVsR=parseFloat(s.stat?.ops)||null;});}catch(e){}
  // FORME OFFENSIVE RÉCENTE : OPS sur les 15 derniers jours
  try{const e=new Date(),st=new Date(Date.now()-15*864e5),f=d=>d.toISOString().split('T')[0];const dr=await api(`/teams/${teamId}/stats?stats=byDateRange&group=hitting&season=${SEASON}&startDate=${f(st)}&endDate=${f(e)}`);const s=dr.stats?.[0]?.splits?.[0]?.stat;if(s&&s.atBats>=20)out.opsRecent=parseFloat(s.ops)||null;}catch(e){}
  return out;
}
...
function confScoreOU(aData,hData,meteo,expTotal,line,fairOver){
  ...
  if(aData.fipPartant!=null)dataC+=25;if(hData.fipPartant!=null)dataC+=25;
  if(aData.ops!=null)dataC+=15;if(hData.ops!=null)dataC+=15;if(meteo)dataC+=10;
  dataC=Math.min(100,dataC);
  const conv=Math.min(100,Math.abs(expTotal-line)/2*100); // 2 runs d'écart = conviction max
  const qual=fairOver!=null?75:45; // cotes Over+Under cohérentes dispo = marché lisible
  return Math.round(dataC*0.4+conv*0.4+qual*0.2);
}
// Confiance RUN LINE : dérivée de la confiance victoire (même modèle) + caractère décisif de la couverture.
function confScoreRL(cScoreWin,coverProba){return Math.round(cScoreWin*0.6+Math.min(100,Math.abs(coverProba-50)*3)*0.4);}
// Intervalle de proba (±) déduit de la confiance : plus c'est confiant, plus c'est resserré.
function margeFromConf(c){return Math.max(3,Math.min(12,Math.round(13-c/10)));}
...
function calcEquipe(data){
  ...
  if(pyth!=null){add(sWinPct(pyth),0.14,{nom:'Pourcentage de victoires théorique',valeur:`${Math.round(pyth*100)}% de victoires attendues cette saison`,detail:'Calculé sur les points marqués et encaissés réels (formule pythagoricienne de Bill James, 1977). Poids réduit (0.14) : il recoupe l\'attaque (OPS) et la défense (ERA équipe), déjà comptées séparément — validé au backtest.',code:'pyth',valVars:{pct:Math.round(pyth*100)},sense:sWinPct(pyth)>=55?'pos':sWinPct(pyth)<=45?'neg':'neu'});}
  else if(data.winPct!=null){add(sWinPct(data.winPct),0.22,{nom:'Pourcentage de victoires cette saison',valeur:`${Math.round((data.winPct||0.5)*100)}% de victoires en 2026`,detail:'Bilan victoires/défaites de la saison',code:'winpct',valVars:{pct:Math.round((data.winPct||0.5)*100)},sense:sWinPct(data.winPct)>=55?'pos':sWinPct(data.winPct)<=45?'neg':'neu'});}
  if(data.eraPartant!=null||data.fipPartant!=null){const metric=data.fipPartant??data.eraPartant;const s=zScore(metric,LG.starterFip||LG.starterEra,true)??sERA(metric);const q=metric<2.8?'exceptionnel':metric<3.4?'excellent':metric<3.9?'très bon':metric<4.5?'correct':'faible';const qK=metric<2.8?'q_excep':metric<3.4?'q_excel':metric<3.9?'q_vgood':metric<4.5?'q_correct':'q_weak';const eraStr=data.eraPartant!=null?` (ERA ${data.eraPartant.toFixed(2)})`:'';add(s,0.26,{nom:'Qualité du lanceur de départ',valeur:`${data.nomPartant||'Lanceur'} : FIP ${metric.toFixed(2)} (${q})${eraStr}`,detail:'Le FIP isole la vraie performance du lanceur (circuits, marches, retraits) de la défense derrière lui — plus prédictif que l\'ERA. Le partant influence ~60% du match.',code:'starter',valVars:{name:data.nomPartant||'Lanceur',fip:metric.toFixed(2),qKey:qK,eraStr},sense:s>=65?'pos':s<=40?'neg':'neu'});}
  if(data.ops!=null){const s=zScore(data.ops,LG.ops,false)??sOPS(data.ops);const q=data.ops>0.820?'excellente':data.ops>0.740?'bonne':data.ops>0.670?'correcte':'faible';const qK=data.ops>0.820?'qo_excel':data.ops>0.740?'qo_good':data.ops>0.670?'qo_correct':'qo_weak';const ctx=data.opsSplit?` à ${data.estDomicile?'domicile':'l\'extérieur'}`:'';add(s,0.25,{nom:'Force offensive de l\'équipe',valeur:`${data.ops}${ctx} — attaque ${q}`,detail:data.opsSplit?`Statistique offensive réelle ${data.estDomicile?'à domicile':'en déplacement'} (split contextuel)`:'Mesure la capacité à atteindre les bases et la puissance de frappe combinées',code:'ops',valVars:{ops:data.ops,ctxKey:data.opsSplit?(data.estDomicile?'ctx_home':'ctx_away'):'',qKey:qK},detKey:data.opsSplit?(data.estDomicile?'fac_ops_d_home':'fac_ops_d_away'):'fac_ops_d',sense:s>=58?'pos':s<=42?'neg':'neu'});}
  if(data.whipPartant!=null){const s=zScore(data.whipPartant,LG.whip,true)??sWHIP(data.whipPartant);const poids=data.eraPartant!=null?0.04:0.10;add(s,poids,{nom:'Efficacité du lanceur (marcheurs + coups sûrs/manche)',valeur:`${data.whipPartant.toFixed(2)} — ${s>=65?'très efficace':s<=40?'peu efficace':'efficacité correcte'}${data.eraPartant!=null?' · poids réduit (corrélé à l\'ERA)':''}`,detail:'Mesure combien de coureurs le lanceur laisse atteindre les bases par manche. Fortement corrélé à l\'ERA, son poids est réduit pour éviter le double comptage.',code:'whip',valVars:{whip:data.whipPartant.toFixed(2),qKey:s>=65?'qw_hi':s<=40?'qw_lo':'qw_mid',reducedKey:data.eraPartant!=null?'fac_whip_reduced':''},sense:s>=65?'pos':s<=40?'neg':'neu'});}
  if(data.k9Partant!=null){const s=zScore(data.k9Partant,LG.k9,false)??sKper9(data.k9Partant);add(s,0.08,{nom:'Capacité d\'élimination directe du lanceur',valeur:`${data.k9Partant.toFixed(1)} éliminations directes par manche en moyenne`,detail:'Un lanceur qui élimine beaucoup = moins de risques pour son équipe',code:'k9',valVars:{k9:data.k9Partant.toFixed(1)},sense:s>=65?'pos':s<=40?'neg':'neu'});}
  if(data.eraEquipe!=null){const s=zScore(data.eraEquipe,LG.teamEra,true)??sERA(data.eraEquipe);add(s,0.13,{nom:'Qualité défensive globale de l\'équipe',valeur:`${data.eraEquipe.toFixed(2)} points encaissés/match en moyenne (tous lanceurs)`,detail:'Reflète la qualité du bullpen (lanceurs de remplacement) en plus du partant',code:'teamera',valVars:{era:data.eraEquipe.toFixed(2)},sense:s>=58?'pos':s<=42?'neg':'neu'});}
  // PLATOON : OPS de l'attaque face à la MAIN du lanceur adverse (avantage de matchup)
  if(data.opsVsHand!=null&&data.oppHand){const s=zScore(data.opsVsHand,LG.ops,false)??sOPS(data.opsVsHand);const hd=data.oppHand==='L'?'gaucher':'droitier';add(s,0.10,{nom:`Avantage platoon (vs lanceur ${hd})`,valeur:`OPS ${data.opsVsHand.toFixed(3)} face aux ${hd==='gaucher'?'gauchers':'droitiers'}`,detail:'Performance offensive réelle de l\'équipe contre la main du lanceur adverse — une équipe frappe souvent bien mieux contre une main que contre l\'autre.',code:'platoon',valVars:{ops:data.opsVsHand.toFixed(3),handKey:data.oppHand==='L'?'hand_lhp':'hand_rhp'},sense:s>=58?'pos':s<=42?'neg':'neu'});}
  // FORME OFFENSIVE RÉCENTE : OPS des 15 derniers jours (séries chaudes/froides)
  if(data.opsRecent!=null){const s=zScore(data.opsRecent,LG.ops,false)??sOPS(data.opsRecent);add(s,0.05,{nom:'Forme offensive récente (15 j)',valeur:`OPS ${data.opsRecent.toFixed(3)} sur 15 jours${data.ops?` (saison ${(+data.ops).toFixed(3)})`:''}`,detail:'L\'attaque sur les 15 derniers jours capte les séries chaudes/froides, souvent loin de la moyenne de saison.',code:'recoff',valVars:{ops:data.opsRecent.toFixed(3),season:data.ops?(+data.ops).toFixed(3):'—'},sense:s>=58?'pos':s<=42?'neg':'neu'});}
  // FORME RÉCENTE DU LANCEUR : ERA des 30 derniers jours
  if(data.eraRecent!=null){const s=zScore(data.eraRecent,LG.starterEra,true)??sERA(data.eraRecent);add(s,0.05,{nom:'Forme récente du lanceur (30 j)',valeur:`ERA ${data.eraRecent.toFixed(2)} sur 30 jours`,detail:'L\'ERA récente du partant (30 derniers jours) reflète sa forme actuelle, parfois éloignée de sa moyenne de saison.',code:'recpit',valVars:{era:data.eraRecent.toFixed(2)},sense:s>=58?'pos':s<=42?'neg':'neu'});}
  {const s=data.estDomicile?58:42;scoreTotal+=s*0.06;totalPoids+=0.06;scoresContrib.push({score:s,poids:0.06});facteurs.push({nom:data.estDomicile?'Avantage de jouer à domicile':'Désavantage de jouer à l\'extérieur',valeur:data.estDomicile?'🏠 Reçoit chez lui — avantage statistique de ~54% en MLB':'✈️ En déplacement — légèrement désavantagé statistiquement',detail:'En MLB, l\'équipe qui reçoit gagne en moyenne 54% de ses matchs à domicile',code:data.estDomicile?'home':'away',score:s,poids:6,sense:data.estDomicile?'pos':'neg'});}
  if(data.forme&&data.forme.total>=3){const fPct=data.forme.formePct;const s=Math.round(Math.max(15,Math.min(90,30+fPct*60)));add(s,0.05,{nom:'Forme récente (10 derniers matchs réels)',valeur:`${data.forme.wins}V-${data.forme.total-data.forme.wins}D sur les ${data.forme.total} derniers matchs`,detail:'Résultats réels récupérés via l\'API MLB, pas une estimation',code:'form',valVars:{w:data.forme.wins,l:data.forme.total-data.forme.wins,total:data.forme.total},sense:fPct>=0.6?'pos':fPct<=0.4?'neg':'neu'});}
  ...
  const dataDispos=Math.round(Math.min(100,totalPoids/1.32*100)); // normalisé sur le poids max possible (incl. platoon+forme récente) → discrimine la vraie complétude
...
function estimerMatch(aData,hData,parkPct,vegas,scoreA,scoreH){
  ...
  const pitchRating=d=>{let fip=d.fipPartant??d.eraPartant??leaguePitch;if(d.eraRecent!=null)fip=0.7*fip+0.3*d.eraRecent;/* forme récente du partant */const te=d.eraEquipe??leaguePitch;const raw=0.55*fip+0.45*te;return leaguePitch+SH*(raw-leaguePitch);};
  const offRatio=d=>{const parts=[[d.ops??leagueOPS,1]];if(d.opsVsHand!=null)parts.push([d.opsVsHand,1]);/* platoon vs main adverse */if(d.opsRecent!=null)parts.push([d.opsRecent,0.5]);/* forme récente */const sw=parts.reduce((a,p)=>a+p[1],0),eff=parts.reduce((a,p)=>a+p[0]*p[1],0)/sw;const raw=Math.pow(eff/leagueOPS,1.6);return 1+SH*(raw-1);};
...
async function build(){
  ...
  await Promise.all(pitcherIds.map(async id=>{try{
    const d=await api(`/people/${id}/stats?stats=season&group=pitching&season=${SEASON}`);const s=d.stats?.[0]?.splits?.[0]?.stat;
    if(s){
      pitcherMap[id]=s;
      try{const ph=await api(`/people/${id}`);s._hand=ph.people?.[0]?.pitchHand?.code||null;}catch(e){}  // main du lanceur (R/L) pour le platoon
      try{const e2=new Date(),st=new Date(Date.now()-30*864e5),f=d=>d.toISOString().split('T')[0];const dr=await api(`/people/${id}/stats?stats=byDateRange&group=pitching&season=${SEASON}&startDate=${f(st)}&endDate=${f(e2)}`);const rs=dr.stats?.[0]?.splits?.[0]?.stat;if(rs&&parseFloat(rs.inningsPitched)>=8)s._eraRecent=parseFloat(rs.era)||null;}catch(e){}  // ERA des 30 derniers jours
    }
  }catch(e){}}));
  ...
  for(const game of games){
    ...
    // RECALIBRATION À LA SOURCE : on corrige la proba de victoire du favori puis on redérive le split (les 2 côtés somment à 100).
    // Résultat : victoire, O/U et run line partent TOUS de la même proba de victoire → fini l'incohérence 46% (headline) vs 51.7% (barres/facteurs/bandeau value).
    if(recal.active){const favRaw=Math.max(pA,pH),favCal=pt(recal.apply(favRaw));if(pH>=pA){pH=favCal;pA=pt(100-pH);}else{pA=favCal;pH=pt(100-pA);}}
    ...
    const favProbaRaw=favH?pHraw:pAraw;
    ...
    else{const fw=favH?hStand.winPct:aStand.winPct,dw=favH?aStand.winPct:hStand.winPct;if(fw!=null&&dw!=null&&fw<dw-0.03&&favProba>52){valueAlert=`Value contrarian : ${favTeam.name} a un moins bon bilan saison (${Math.round(fw*100)}%) que l'adversaire (${Math.round(dw*100)}%), mais le modèle le favorise (${favProba.toFixed(1)}%).`;valueData={kind:'contrarian',team:favTeam.name,fw:Math.round(fw*100),dw:Math.round(dw*100),proba:favProba.toFixed(1)};}}
    ...
    // Données structurées (i18n côté client ; le HTML FR ci-dessus sert de repli)
    const meteoData=meteo?{dome:meteo.toit==='fixe',stade:meteo.stade,temp:meteo.temp,humidity:meteo.humidity,wind:Math.round(meteo.wind),score:wind.score,impact:wind.impact}:null;
    const compoData=partantManquant?{kind:'missing'}:(!compoConfirmee?{kind:'unconfirmed',h:heuresAvant}:null);
    const fatigueData=favForme&&(favForme.reposSemaine===0||favForme.doubleHeaders>0)?{m7:favForme.matchs7j,dh:favForme.doubleHeaders}:null;
    const regData=favRes.regInfo?{team:favTeam.name,n:favRes.regInfo.n}:null;
    const tendDataV=tf?{kind:'win',team:favTeam.name,bw:tf.bilan10.w,bl:tf.bilan10.l,splitSide:favH?(tf.domicile.n?'home':''):(tf.exterieur.n?'away':''),splitW:favH?tf.domicile.w:tf.exterieur.w,splitL:favH?tf.domicile.l:tf.exterieur.l,serieW:tf.serieW}:null;
    ...
    const base={gamePk:game.gamePk,heure:time,dateCourt,gameDate:game.gameDate,matchLabel:matchLbl,awayTeam:slim(away.team),homeTeam:slim(home.team),probaAway:pA,probaHome:pH,state:game.status.abstractGameState,meteoHTML,compoHTML,meteoData,compoData};
    // 1. VICTOIRE
    pronos.push({...base,id:`${game.gamePk}-V`,type:'victoire',typeLabel:'Victoire du match',pickKey:'pk_win',pickVars:{team:favTeam.name},explKey:'expl_win',explVars:{team:favTeam.name,proba:favProba.toFixed(1),lo:Math.max(0,favProba-margeCI).toFixed(0),hi:Math.min(100,favProba+margeCI).toFixed(0),ra:sim.expRunsA.toFixed(1),rh:sim.expRunsH.toFixed(1)},pick:`${favTeam.name} remporte le match`,explication:`Modèle sur données réelles (stats MLB + cotes du marché) : ${favTeam.name} a ${favProba.toFixed(1)}% de chances de gagner (intervalle ${Math.max(0,favProba-margeCI).toFixed(0)}–${Math.min(100,favProba+margeCI).toFixed(0)}%). Runs estimés : ${sim.expRunsA.toFixed(1)} − ${sim.expRunsH.toFixed(1)}.`,proba:favProba,probaRaw:favProbaRaw,confiance:conf,confScore:cScore,marge:margeCI,coteVegas:coteVegasFav,bookmaker:bkFav,fairProba:fairFav,coteUS:(favH?vg.coteH_us:vg.coteA_us)??null,bkUS:(favH?vg.bkH_us:vg.bkA_us)??null,coteEU:(favH?vg.coteH_eu:vg.coteA_eu)??null,bkEU:(favH?vg.bkH_eu:vg.bkA_eu)??null,facteurs:favRes.facteurs,dataDispos:Math.round(Math.min(rA.dataDispos,rH.dataDispos)),regAlertHTML,valueAlertHTML,formeDots,fatigueBadge,tendance:tendV,tendData:tendDataV,fatigueData,regData,valueData,scoreA:rA.score,scoreH:rH.score});
    // 2. TOTAL
    ...
    if(vegas&&vegas.spHomePoint!=null&&vegas.spAwayPoint!=null){
      ...
      const rlProbaRaw=side.proba;if(recal.active)side.proba=pt(recal.apply(side.proba)); // couverture recalibrée (cohérente avec la proba de victoire, elle-même recalibrée)
      ...
      pronos.push({...base,id:`${game.gamePk}-RL`,type:'runline',typeLabel:'Run line (handicap)',pickKey:'pk_rl',pickVars:{team:side.team.name,point:(side.point>0?'+':'')+side.point},explKey:'expl_rl',explVars:{condKey:side.point<0?'expl_rl_neg':'expl_rl_pos',team:side.team.name,proba:side.proba.toFixed(1)},pick:`${side.team.name} ${side.point>0?'+':''}${side.point}`,rlIsHome:side.isHome,rlPoint:side.point,explication:`Run line : ${side.point<0?`${side.team.name} doit gagner par 2 points ou plus`:`${side.team.name} peut perdre par 1 point max (ou gagner)`}. Couverture estimée : ${side.proba.toFixed(1)}%.`,proba:pt(side.proba),probaRaw:rlProbaRaw,confiance:Math.max(1,conf-1),confScore:csRL,marge:margeRL,coteVegas:side.odds,bookmaker:side.bk,coteUS:(side.isHome?vg.spHomeOdds_us:vg.spAwayOdds_us)??null,bkUS:(side.isHome?vg.bkSpHome_us:vg.bkSpAway_us)??null,coteEU:(side.isHome?vg.spHomeOdds_eu:vg.spAwayOdds_eu)??null,bkEU:(side.isHome?vg.bkSpHome_eu:vg.bkSpAway_eu)??null,fairProba:side.fair,dataDispos:Math.round(Math.min(rA.dataDispos,rH.dataDispos)),facteurs:[{nom:`Probabilité de victoire de ${side.team.name}`,valeur:`${side.win.toFixed(1)}%`,detail:'Issue du modèle sur données réelles',code:'rlwin',valVars:{team:side.team.name,win:side.win.toFixed(1)},score:side.win,poids:50,sense:side.win>=55?'pos':'neg'},{nom:'Couverture du handicap estimée',valeur:`${side.proba.toFixed(1)}%`,detail:side.point<0?'Probabilité que l\'équipe gagne par 2 points ou plus':'Probabilité que l\'équipe ne perde pas par 2+',code:'rlcover',valVars:{proba:side.proba.toFixed(1)},detKey:side.point<0?'fac_rlcover_d_neg':'fac_rlcover_d_pos',score:side.proba,poids:50,sense:side.proba>=55?'pos':'neg'}],regAlertHTML:'',tendance:(()=>{const sf=(side.isHome?hForme:aForme)?.tendances;if(!sf)return '';return `📈 ${side.team.name} récemment : ${side.point<0?`gagne par 2+ dans ${sf.cover15}/${sf.over85n}`:`couvre le +1.5 dans ${sf.plus15}/${sf.over85n}`}`;})(),tendData:(()=>{const sf=(side.isHome?hForme:aForme)?.tendances;if(!sf)return null;return {kind:'rl',team:side.team.name,coverKind:side.point<0?'win2':'cover',x:side.point<0?sf.cover15:sf.plus15,y:sf.over85n};})()});
    }
  }
...
// Exports pour le backtest (réutilise le VRAI moteur, pas une copie)
export { calcEquipe, normProba, calcFIP, pythagorean, zScore, statsLigue, LG, estimerMatch, PARK };
